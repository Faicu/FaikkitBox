      ...
      {data?.status === "error" &&
        (recovering ? (
          <div className="rounded-2xl border border-sky-500/30 bg-sky-500/10 p-3 text-sm text-sky-300">
            Immich se repornește și va reveni online în câteva momente.
          </div>
        ) : (
      ...
      {data?.status === "ok" && (
        <>
          <div className="grid grid-cols-2 gap-2">
            <StatCard
              label="Total fișiere"
              value={(data.totalAssets ?? 0).toLocaleString()}
              icon={<Activity className="h-4 w-4" />}
              accent="text-purple-400"
            />
            <StatCard
              label="Spațiu folosit"
              value={formatBytes(data.usageBytes ?? 0)}
              icon={<HardDrive className="h-4 w-4" />}
              accent="text-purple-400"
            />
            <StatCard
              label="Fotografii"
              value={(data.photos ?? 0).toLocaleString()}
          ...
          {(data.uploadsToday != null ||
            data.uploadsThisWeek != null ||
            data.jobQueueDepth != null) && (
                  ...
                  <li key={i} className="flex items-center justify-between px-3 py-2 text-sm">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {u.photos.toLocaleString()} fotografii · {u.videos.toLocaleString()}{" "}
                        videoclipuri
                      </div>
                    </div>
