...
function stateBadge(state: string) {
  ...
  if (
    (s.includes("up") || s === "uploading" || s === "stalledup") &&
    !s.includes("paus") &&
    !s.includes("stop")
  )
    return { text: "Seed", cls: "bg-emerald-500/20 text-emerald-400" };
  if (s.includes("paus") || s.includes("stop"))
    return { text: "Oprit", cls: "bg-muted text-muted-foreground" };
...
function QbitPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (res, vars) => {
      ...
      const label =
        vars.action === "pause" ? "Oprite" : vars.action === "delete" ? "Șterse" : "Reluate";
      ...
      {data?.status === "error" &&
        (recovering ? (
          <div className="rounded-2xl border border-sky-500/30 bg-sky-500/10 p-3 text-sm text-sky-300">
            qBittorrent se repornește și va reveni online în câteva momente.
          </div>
        ) : (
          <ErrorCard
            title="qBittorrent indisponibil"
      ...
      {data?.status === "ok" && (
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            <StatCard
              label="Descărcare"
              value={formatSpeed(data.dlSpeed)}
              sub={`Total ${formatBytes(data.totalDl)}`}
              icon={<ArrowDown className="h-4 w-4" />}
              accent="text-sky-400"
            />
            <StatCard
              label="Încărcare"
              value={formatSpeed(data.upSpeed)}
              sub={`Total ${formatBytes(data.totalUp)}`}
            ...
            <StatCard
              label="Spațiu liber"
              value={formatBytes(data.freeSpaceOnDisk)}
          ...
          {(data.alltimeDl != null || data.alltimeUp != null) && (
            <div className="grid grid-cols-2 gap-2">
              <StatCard
                label="Total descărcat"
                value={formatBytes(data.alltimeDl ?? 0)}
                icon={<ArrowDown className="h-4 w-4" />}
                accent="text-sky-400"
              />
              <StatCard
                label="Total încărcat"
                value={formatBytes(data.alltimeUp ?? 0)}
                sub={`Rație totală ${(() => {
